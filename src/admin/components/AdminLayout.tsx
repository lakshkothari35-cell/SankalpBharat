        ...
        <header className="h-20 border-b border-white/5 flex items-center justify-between px-4 md:px-8 bg-[#0a0705]/50 backdrop-blur-xl z-40">
          ...
          <div className="flex items-center gap-4 md:gap-6 relative" ref={notificationRef}>
            <button 
              ...
              className={`relative p-2 transition-colors group rounded-xl ${showNotifications ? 'bg-gold/10 text-gold' : 'text-beige/40 hover:text-beige'}`}
            >
            ...
            <AnimatePresence>
              {showNotifications && (
                <motion.div
                  ...
                  className="absolute top-14 right-0 w-[320px] sm:w-[400px] bg-[#0c0805] border border-gold/20 rounded-[32px] shadow-2xl overflow-hidden z-[100] backdrop-blur-3xl"
                >
                  <div className="p-6 border-b border-gold/10 flex items-center justify-between bg-maroon/5">
                    <div className="flex items-center gap-3">
                      <Bell className="w-4 h-4 text-gold" />
                      <h3 className="text-sm font-black uppercase tracking-widest">Alert Center</h3>
                    </div>
                    {unreadCount > 0 && (
                      <button 
                        onClick={markAllAsRead}
                        className="text-[9px] font-black uppercase tracking-widest text-gold/40 hover:text-gold transition-colors"
                      >
                        Mark all as read
                      </button>
                    )}
                    ...
                    {notifications.length > 0 ? (
                      <div className="divide-y divide-white/5">
                        {notifications.map((n) => (
                          <div 
                            key={n.id} 
                            className={`p-6 hover:bg-white/[0.02] transition-colors relative group cursor-pointer ${n.unread ? 'bg-gold/[0.02]' : ''}`}
                          >
                            {n.unread && <div className="absolute left-0 top-0 bottom-0 w-1 bg-gold" />}
                            <div className="flex gap-4">
                              ...
                              <div className="flex-1 space-y-1">
                                <div className="flex items-center justify-between gap-2">
                                  <h4 className={`text-xs font-bold ${n.unread ? 'text-beige' : 'text-beige/60'}`}>{n.title}</h4>
                                  <span className="text-[8px] font-black uppercase tracking-widest text-beige/20">{n.time}</span>
                                </div>
                                <p className="text-[11px] text-beige/40 leading-relaxed italic">{n.message}</p>
                              </div>
                            </div>
                          </div>
                        ))}
                      ...
                      <div className="py-12 text-center">
                        <Bell className="w-8 h-8 text-beige/10 mx-auto mb-4" />
                        <p className="text-[10px] text-beige/20 font-black uppercase tracking-widest">No new alerts</p>
                      </div>
                    )}
                  </div>

                  <div className="p-4 border-t border-gold/10 text-center bg-maroon/5">
                    <button className="text-[10px] font-black uppercase tracking-widest text-gold hover:translate-x-1 transition-transform inline-flex items-center gap-2">
